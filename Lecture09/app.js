function Chatroom() {
    this.messages = [];
    this.users = [];
    this.currentUser = null;
}

Chatroom.prototype.login = function () {
    var provider = new firebase.auth.GoogleAuthProvider();
    firebase
        .auth()
        .signInWithPopup(provider)
        .then(function (result) {
            // This gives you a Google Access Token. You can use it to access the Google API.
            var token = result.credential.accessToken;
            // The signed-in user info.
            this.currentUser = result.user;
            // ...
            db
                .collection("users")
                .doc(uid)
                .set({
                    email: this.currentUser.email,
                    displayName: this.currentUser.displayName,
                    photoURL: this.currentUser.photoURL
                });

            var loginMessage = {
                content: `${this.currentUser.displayName} 上線囉`,
                user: {
                    displayName: "系統訊息",
                    photoURL: "data:image/png;base64,"

                },
                dateTime: moment().format("YYYYMMDD-hh:mm:ss")
            };
            this.addMessage(loginMessage);
        }).catch(function (error) {
            // Handle Errors here.
            var errorCode = error.code;
            var errorMessage = error.message;
            // The email of the user's account used.
            var email = error.email;
            // The firebase.auth.AuthCredential type that was used.
            var credential = error.credential;
            // ...
        });
};

Chatroom.prototype.logout = function () {
    firebase.auth().signOut().then(function () {
        // Sign-out successful.
        db.doc(`users/${this.currentUser.uid}`).delete();
        this.currentUser = null;
    }).catch(function (error) {
        // An error happened.
    });
};


Chatroom.prototype.addMessage = function (message) {
    db
        .collection("/chatroom/QCCiCa99uwNZolbFw3y6/chats")
        .add(message);
};

var chatroom = new Chatroom();
// 登入按鈕
$("#login").click(function () {
    chatroom.login();
});


var $chats = $("#messages");

db
    .collection("/chatroom/QCCiCa99uwNZolbFw3y6/chats")
    .orderBy("dateTime", "asc")
    .onSnapshot(function (snapshot) {
        snapshot.docChanges().forEach(function (change) {
            if (change.type === "added") {
                console.log(change.doc.data());
                var chat = change.doc.data();
                var isCurrentUser = chatroom.currentUser.displayName == chat.user.displayName;
                var chatHTML =
                    `<div id="${change.doc.id}" class="message ${isCurrentUser? "current-user": ""}">
                <img src="${chat.user.photoURL}" alt="" class="user-photo">
                <div class="message-content">${chat.content}</div>
            </div>`;
                var $chat = $(chatHTML);
                $chat.appendTo($chats);


            }
        });
    });

// 判斷登入狀態
firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
        // User is signed in.
        chatroom.currentUser = user;
        var displayName = user.displayName;
        var email = user.email;
        var emailVerified = user.emailVerified;
        var photoURL = user.photoURL;
        var isAnonymous = user.isAnonymous;
        var uid = user.uid;
        var providerData = user.providerData;
        $("#login").addClass("hidden");
        $("#chat-editor").removeClass("hidden");

        var message = {
            content: `${displayName} 上線囉`,
            user: {
                displayName: "系統訊息",
                photoURL: "data:image/png;base64,"

            },
            dateTime: moment().format("YYYYMMDD-hh:mm:ss")
        };

        chatroom.addMessage(message);

        db
            .collection("users")
            .doc(uid)
            .set({
                email: user.email,
                displayName: user.displayName,
                photoURL: user.photoURL
            });
    } else {
        // User is signed out.
        // ...
        $("#chat-editor").addClass("hidden");
        $("#login-btn").removeClass("hidden");
        var message = {
            content: `${currentUser.displayName} 離開聊天室`,
            user: {
                displayName: "系統訊息",
                photoURL: "data:image/png;base64,"
            },
            dateTime: moment().format("YYYYMMDD-hh:mm:ss")

        };
        chatroom.addMessage(message);
    }
});

$("#chat-editor").keydown(function (e) {
    db
        .doc("/chatroom/QCCiCa99uwNZolbFw3y6")
        .update({
            typing: true
        });
});

$("#chat-editor").keyup(function (e) {
    console.log(e.keyCode)
    if (e.keyCode == 13) {
        db
            .doc("/chatroom/QCCiCa99uwNZolbFw3y6")
            .update({
                typing: false
            });

        var message = {
            content: $("#chat-editor").val(),
            user: {
                displayName: chatroom.currentUser.displayName,
                photoURL: chatroom.currentUser.photoURL
            },
            dateTime: moment().format("YYYYMMDD-hh:mm:ss")
        };
        chatroom.addMessage(message);

        $("#chat-editor").val("");
        $chats.animate({
            scrollTop: $chats.height() * 30
        }, 1000);

    }

});

db
    .doc("/chatroom/QCCiCa99uwNZolbFw3y6")
    .onSnapshot(function (snapshot) {
        if (snapshot.data().typing) {
            $("#typing").removeClass("hidden");
        } else {
            $("#typing").addClass("hidden");
        }
    });

$("#scroll-btn").click(function () {
    $chats.animate({
        scrollTop: $chats.height() * 30
    }, 1000);
})